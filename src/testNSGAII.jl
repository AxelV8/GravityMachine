...
function NSGAII_GM_SolAdmissible(c1, c2, A, Listeinitx)
    nbctr = size(A,1)
    nbvar = size(A,2)
    ...
    function CV(X)
        ...
        for i in 1:nbctr
            ...
            if sum!=1
                sum == 0 ? sumReturn+=2 : sumReturn+=sum
            end
        end
        return sumReturn
    end

    init()=toBit(rand(Listeinitx), nbvar)

    SEED=[]
    for sol in Listeinitx
        push!(SEED, toBit(sol, nbvar))
    end

    println("==================NSGA II==================")


    result = nsga(100, 50, z, init, fCV = CV, seed = SEED)
